...
Fishing trip identifier: unique identifier of a fishing trip. Fishing trips have to be identified before the anonymisation process because after a spatial translation it won’t be possible to link ourdata to a harbour or coastline.
...
Space The ourdata have to be randomly translated. To ensure that the country or harbours of origin won’t be possibly recognized while keeping the geographic information undisturbed for one vessel, it will be randomly translated for each fishing vessel.
...
The dataset has been resampled to a constant timelapse of 15 minutes between ourdata.
...
lonlat2UTM
Function that returns the appropriated UTM system based on lon/lat ourdata. From Robin Lovelace, Geocomputation with R
...
Translate_ourdata
Function to translate ourdata in space
...
positions<-read.table("dataset_SSF_WKSSFGEO2_CNR.csv", sep=",", header = TRUE) # your data
positions <- positions[,-1]

positions$BoatID <- factor(as.character(positions$BoatID)) #check the name of your vessel ID column

VE.key <- data.frame(BoatID = levels(positions$BoatID), 
                     VE_ANO = Harmonize_Ids("VE", 1:nlevels(positions$BoatID), ID.length = 5)) 

levels(positions$BoatID) <- VE.key$VE_ANO
positions$BoatID <- as.character(positions$BoatID)

### Check the results
table(positions$BoatID)#BoatID is now your anonymised vessel name
...
positions$DATE_TIME<-as.POSIXct(paste(positions$Date, positions$Time),format="%d/%m/%Y %H:%M")
...
PosSf.former <- PosSf.former[,!colnames(PosSf.former) %in% c("Date","Time")]


boats <- unique(positions$BoatID)
boat <- boats[1]

Position.trans <- lapply(boats, function(boat){

    key.spatial <- sample(-2000:2000, 1)*1000
    new.pos <- Translate_Positions(sf.points = PosSf.former[ PosSf.former$BoatID %in% boat, ],
                                 Key.x = key.spatial,
                                 Key.y = key.spatial
                                 )  
...
write.csv(positions.trans, "ITA_SSF_WKSSFGEO2_CNR_Annon.csv", row.names = FALSE )
```
